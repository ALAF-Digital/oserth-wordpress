...
<?php if ( $has_orders ) : ?>
	<div class="main-account-inner-container scroll-01">
		...
		<div class="main-order-list-container scroll">
			<?php /* ?><div class="main-heading-list">
				<ul>
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<li class="woocommerce-orders-table__header woocommerce-orders-table__header-<?php echo esc_attr( $column_id ); ?>"><span class="nobr"><?php echo esc_html( $column_name ); ?></span></li>
					<?php endforeach; ?>
				</ul>
			</div>
							...
							<li class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
								...
								<?php elseif ( 'order-actions' === $column_id ) : ?>
									...
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										...
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button order-btn ' . sanitize_html_class( $key ) . '">' . esc_html( $name ) . '</a>';
									}
								}
								?>
							<?php endif; ?>
